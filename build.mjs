...
for(const folder of await readdir(source,{withFileTypes:true})){if(!folder.isDirectory()||!/^Vcard-[a-z0-9][a-z0-9-]*$/.test(folder.name))continue;for(const file of await readdir(path.join(source,folder.name),{withFileTypes:true})){if(file.isFile()&&allowed.has(file.name))files.push(folder.name+'/'+file.name);}}
